...
const reveals = document.querySelectorAll('.hero-card, .feature-strip, .info-grid > *');
...
async function getApiFeedbacks() {
  if (!USE_API || !ADMIN_TOKEN) return null;
...
async function getFeedbacks() {
  ...
  return Array.isArray(saved) ? saved.map(normalizeFeedback) : [];
}
...
async function updateHomeStats() {
  ...
  if (foot) {
    ...
    foot.href = feedbacks.length ? 'pages/controle.html' : '../public/fredback.html';
  }
}
