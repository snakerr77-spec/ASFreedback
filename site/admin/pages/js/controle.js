...
function redirectToLogin() {
  ...
  login.searchParams.set('redirect', 'admin/pages/controle.html');
...
function requireLogin() {
  ...
  const activeAuth = auth || { user: 'local', name: 'Acesso local', role: 'admin', mode: 'local' };
  const sessionUser = $('#sessionUser');
  if (sessionUser) sessionUser.textContent = `${activeAuth.name || activeAuth.user} • ${CURRENT_UNIT.name}`;
  return activeAuth;
}
...
async function loadFeedbacks() {
  try {
    ...
    console.warn(error);
    state.feedbacks = [];
    if (USE_API && /sessão|acesso negado|login/i.test(error.message || '')) {
      redirectToLogin();
      return;
    }
