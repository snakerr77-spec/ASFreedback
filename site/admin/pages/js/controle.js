...
const ADMIN_TOKEN = window.FEEDBACK_ADMIN_TOKEN || '';
...
function requireLogin() {
  ...
  if (sessionUser) {
    sessionUser.textContent = `${auth.name || auth.user} • ${auth.role === 'admin' ? 'Admin' : 'Colaborador'}`;
  }

  const clearBtn = $('#limparFeedbacks');
  if (clearBtn && auth.role !== 'admin') {
...
async function getApiFeedbacks() {
  ...
  if (!ADMIN_TOKEN) {
    throw new Error('ADMIN_TOKEN não configurado no arquivo public/pages/config.js.');
  }

  const response = await fetch(endpoint('/api/feedbacks'), {
    headers: authHeaders()
  });
...
function updateKpis(data) {
  ...
  $('#kpiNpsDelta').textContent = data.feedbacks.length ? 'baseado em recomendação' : 'sem recomendações ainda';
}
...
function setClearFeedbacks(auth) {
  ...
  btn.addEventListener('click', async () => {
    const message = USE_API
      ? 'Deseja apagar TODOS os feedbacks salvos na Cloudflare? Esta ação não pode ser desfeita.'
    ...
    try {
      if (USE_API) {
        if (!ADMIN_TOKEN) throw new Error('ADMIN_TOKEN não configurado.');
        const response = await fetch(endpoint('/api/feedbacks'), {
          method: 'DELETE',
          headers: authHeaders()
        });
